...
(deftest differentiation
  (testing "without units"
    (is (almost-equal 16 (differentiate (fn [x] (* 2 x x)) 4 []))))
  (testing "with units"
    (is (almost-equal (m 16.0) (differentiate (fn [x] (ops/* 2 x x)) (m 4) [])))
    (is (almost-equal (sec 16) (differentiate (fn [x] (ops/* (sec 2) x x)) 4 [])))
    (is (almost-equal ((inverse m) 16) (differentiate (comp (fn [x] (* 2 x x)) #(getValue % m)) (m 4) [])))
  )
)

(deftest integration
  (testing "without units"
    (is (almost-equal 15.0 (integrate (fn [x] (* x 2)) [1 4] []))))
  (testing "with units"
